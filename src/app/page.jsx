import React from 'react'

export default function Home() {
  return (
    <div>
      Link to <a href="/bmi-calculator" className='text-blue-600 underline'>BMI Calculator</a>
      <br />
      Link to <a href="/tic-tac-toe-game" className='text-blue-600 underline'>Tic Tac Toe Game</a>  
      <br />
      Link to <a href="/quiz-app" className='text-blue-600 underline'>Quiz App</a>  
      <br />
      Link to <a href="/random-password-generator" className='text-blue-600 underline'>Random Password Generator</a>  
      
    </div>
   
  )
}
